...
def _draw_text(c: canvas.Canvas, x: float, y: float, text: str, size: int = 9, bold: bool = False, color=colors.black):
    c.setFillColor(color)
    c.setFont("Helvetica-Bold" if bold else "Helvetica", size)
    c.drawString(x, y, text)


def _draw_paragraph(c: canvas.Canvas, x: float, y: float, text: str, w: float, size: int = 7, leading: float = 8.2):
...
def build_template_pdf(out_path: Path) -> Path:
    ...
    # Informational text (static) + general info blocks.
    info_y = y_fatca - 62
    _draw_text(
        c,
        30,
...
def _coerce_checkbox_values(data: dict[str, Any]) -> dict[str, Any]:
...
def fill_template(template_path: Path, out_path: Path, data: dict[str, Any]) -> Path:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    data = _coerce_checkbox_values(data)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Génère le PDF d'autocertification FATCA/EAI (template + version remplie).")
    parser.add_argument("--template-out", default="documents/exports/fatca_eai_autocertification_template.pdf")
    parser.add_argument("--data", default=None, help="JSON de valeurs (ex: scripts/fatca_eai_autocertification_sample_data.json)")
    parser.add_argument("--filled-out", default=None, help="PDF rempli en sortie (si --data est fourni)")
    args = parser.parse_args()

    template_out = Path(args.template_out)
    build_template_pdf(template_out)

    if args.data:
        data = _load_data(Path(args.data))
        filled_out = Path(args.filled_out) if args.filled_out else template_out.with_name(template_out.stem + "_filled.pdf")
        fill_template(template_out, filled_out, data)
